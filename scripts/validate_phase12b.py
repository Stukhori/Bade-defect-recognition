...
def validate(root: Path, *, require_tracked: bool = True) -> dict[str, Any]:
    ...
    _require("Experimental automatic region proposals" in app_source, "proposal workflow is absent")
    _require("st.info(ZERO_PROPOSAL_MESSAGE)" in app_source, "zero-proposal message is absent")
    _require("I reviewed the selected proposal boxes" in app_source, "review gate copy is absent")
